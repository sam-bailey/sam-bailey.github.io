...
def covariate_adjusted_glm_analysis(sample: pd.DataFrame) -> Tuple[float, float]:
    model = smf.glm(
        formula='converted ~ treatment + C(segment_id)', 
        data=sample, 
...
#     conv_rate_treated = np.array([0.15, 0.4, 0.6]), 
# )
# (
#     pop
...
#     .pipe(covariate_adjusted_ols_analysis)
# )
```
...
for i, effect_scale in enumerate(np.linspace(0.0, 1.0, EFFECT_STEPS)):
    ...
    for j in range(N_ITER):
        ...
        pvalues_covariate_glm[i,j] = (
            samp
            .pipe(random_treatment_assignment, rng)
            .pipe(run_experiment, rng)
            .pipe(covariate_adjusted_glm_analysis)[1]
        )
...
plt.plot(ate, power_covariate_glm, ".-", label="Covariate adjusted GLM")
