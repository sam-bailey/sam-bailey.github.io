...
Pkg.activate("./posts/trimmed_mean_extreme_value_theory")
...
using Random, Distributions, StatsPlots, LaTeXStrings
...
#| fig-cap: "Hypothetical revenue per visitor histogram. Skewed, ;ots of zeros on one side. Massive outliers on the other side."
...
I'm far from an expert in EVT. When writing this post, I started out experimeting with different ways to improve the trimmed mean, and in that process discovered EVT. So what I'm writing here is basically me parroting the things I've read over the last few weeks. Take it with a pinch of salt.
:::
...
If this is true, then perhaps we can model 
...
function rmse(sim::Simulation)
	sqrt(mean((μ(sim.ests) .- sim.μ_true).^2))
end

function stddev(sim::Simulation)
	std(μ(sim.ests))
end

function bias(sim::Simulation)
	mean(μ(sim.ests) .- sim.μ_true)
end
...
#| fig-cap: "Average performance of estimators for a Pareto population distribution, with alpha=1.5 and theta=20. RMSE is the Root Mean Squared Error of the estimates vs the true population mean, the Standard Deviation is the standard deviation of the estimates, and the |Bias| is the absolute value of the difference between the average of the estimates and the true population mean. Ideally we want |Bias|=0 and then RMSE and Standard Deviaton as small as possible."

function plot_stats!(sim::Simulation, label::Base.String)
	y = [abs(bias(sim)), stddev(sim), rmse(sim)]
	x = ["|Bias|", "Standard Deviation", "RMSE"]
	p = plot!(
		x, 
		y,
		label=label,
...
#| fig-cap: "A comparison of the standardised errors of the different estimators for a Pareto population distribution, where the standardised error is the difference between the estimated mean and the population mean, divided by the estimated standard deviation. The t-test assumes that this statistic is approximately t-distributied, with d.o.f = samples size - 1. This expected distribution is shown in red. If the distribution matches, then you can expect the confidence intervals you calculate to have good coverage."
...
function merge_estimates(est1::EstimatorResult, est2::EstimatorResult, n1::Int64, n2::Int64)
	...
	σ = sqrt(est1.σ^2 * w1^2 + est2.σ^2 * w2^2 + (est1.μ + est2.μ)^2 * w1 * w2 / n)
...
function windzorised_estimator(x_in::Vector{Float64}, tail_index::Int64)
	if tail_index <= 1
		return sample_mean_variance_estimator(x_in)
	end
...
struct TrimmingSimulation
	...
	bias::Vector{Float64}
	rmse_normed::Vector{Float64}
	stddev_normed::Vector{Float64}
	...
	function TrimmingSimulation(
		...
		rmse_result = Vector{Float64}(undef, length(tail_quantiles))
		stddev_result = Vector{Float64}(undef, length(tail_quantiles))
		bias_result = Vector{Float64}(undef, length(tail_quantiles))
		rmse_normed_result = Vector{Float64}(undef, length(tail_quantiles))
		stddev_normed_result = Vector{Float64}(undef, length(tail_quantiles))
		...
		for (i, q) in enumerate(tail_quantiles)
			...
			rmse_result[i] = rmse(sim)
			stddev_result[i] = stddev(sim)
			bias_result[i] = bias(sim)
			rmse_normed_result[i] = sqrt(mean(standardized_error(sim).^2))
			stddev_normed_result[i] = std(standardized_error(sim).^2)
...
baseline_trimming_sim = TrimmingSimulation(
	...
	50000,
	500,
	windzorised_estimator, # With trimming = 0 this is the same as untrimmed sample mean and variance 
	[0.0]
)

estimators = [trimmed_estimator, windzorised_estimator, hill_tail_estimator, gpd_tail_estimator]
estimator_names = [
	"Trimmed Mean"
	"Windzorised Mean"
	"Hill Tail Mean"
...
estimator_names = ["Trimmed Mean" "Windzorised Mean" "Hill Tail Mean" "GPD Tail Mean"]
...
plot_sim_metrics!(trimming_sims, baseline_trimming_sim, :rmse)
...
plot_sim_metrics!(trimming_sims, baseline_trimming_sim, :stddev)
...
hline!([0.0], color=:black, label="Ideal")

p4 = plot(
	title="Standardised RMSE",
	legend=false,
	xlabel="Trimming Quantile",
	yscale=:log10,
	titlefontsize=10, 
...
plot_sim_metrics!(trimming_sims, baseline_trimming_sim, :rmse_normed)
...
```{julia}
#| label: fig-trimming-sim-by-method
#| fig-cap: "How the different components (Bias and Standard Deviation) contribute towards the overall RMSE of the estimators."

function plot_rmse_components!(sim::TrimmingSimulation)
	plot!(sim.tail_quantiles, sim.stddev, label="Stddev", marker=true, markersize=2)
	plot!(sim.tail_quantiles, abs.(sim.bias), label="|Bias|", marker=true, markersize=2)
	plot!(sim.tail_quantiles, sim.rmse, label="RMSE", marker=true, markersize=2)
end
...
plot([
	begin
		p = plot(
			...
			yscale=:log10, 
			xscale=:log10,
			title=estimator_names[i],
			legend=i==4,
			titlefontsize=10, 
			xlabelfontsize=8,
			xformatter = percent_fmt, 
			xticks=[0.001, 0.00300001, 0.01, 0.0300001, 0.1, 0.300001]
		)
		plot_rmse_components!(sim)
		p
	end
	for (i, sim) in enumerate(trimming_sims)
]...)
```