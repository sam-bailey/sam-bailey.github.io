...
First I need to fit a KDE to some data:
...
Now I can combine my KDE and Pareto distributions to create my pareto tail adjusted estimator.
...
function step!(kde_mix::KDEParetoMixture)
	...
	tail_index = findfirst(p_tail .<= 0.5)
	p_tail[tail_index+1:length(p_tail)] .= 0.0

	kde = LooKDE(kde_mix.x, 1.0 .- p_tail)
	fit!(kde)

	pareto = fit_pareto(kde_mix.log_x[1:tail_index], p_tail[1:tail_index])

	kde_mix.pareto = pareto
	kde_mix.kde = kde
	kde_mix.p_tail = p_tail
	kde_mix.tail_index = tail_index
end

function plot_kde_mix(kde_mix::KDEParetoMixture)
	x_tail = kde_mix.x[1:kde_mix.tail_index]
	x_non_tail = kde_mix.x[(kde_mix.tail_index+1):length(kde_mix.x)]
	println(length(kde_mix.x))
	println(length(x_tail))
	println(length(x_non_tail))
