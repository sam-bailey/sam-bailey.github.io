...
data_dist = MixtureModel(
	[
	   LogNormal(2.0, 0.5),
	   GeneralizedPareto(0.0, 20.0, 0.7)
	], 
	[0.97, 0.03]
);
...
m = get_tail_index(length(x_samples))
...
#| fig-cap: "How the data points have changed after smoothing. Points that fall on the y=x line (black) are unchanged."
...
plot(x_samples, x_samples_smoothed, seriestype=:scatter, label="Data")
plot!(x_range, x_range, color=:black, linestyle=:dash, label="y=x")
